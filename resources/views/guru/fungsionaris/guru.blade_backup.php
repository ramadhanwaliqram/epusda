...
@section('describ')
    Ini adalah halaman guru untuk guru
@endsection
    ...
    <script>
        $(document).ready(function () {
            ...
            var table = $('#order-table').DataTable({
                processing:true,
                serverSide: true,
                ajax: "{{ route('guru.fungsionaris.guru') }}?req=table",
                columns:[
                    {data: 'nip'},
                    {data: 'nama_guru'},
                    {data: 'no_telepon'},
            ...
            form.submit(ev => {
                ...
                $.ajax({
                    url: "{{ route('guru.fungsionaris.guru.write') }}?req=write",
                    cache: false,
                    method: "POST",
                    data: formData,
                    processData: false,
            ...
            $("#order-table").on('click', '.btn-delete', function(ev, data) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('guru.fungsionaris.guru.write') }}?req=delete&id=" + id,
                            cache: false,
                            method: "POST",
                            processData: false,
                            contentType: false,
            ...
            $("#order-table").on('click', '.btn-edit', function(ev, data) {
                var id = ev.currentTarget.getAttribute('data-id');
                $.get("{{route('guru.fungsionaris.guru')}}?req=single&id=" + id, function (data, status){
                    for(key in data) {
                        $(`#${key}`).val(data[key])
                    }
